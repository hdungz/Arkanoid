package com.arkanoid.view;

import com.arkanoid.utils.AssetsManager;
import javafx.animation.ScaleTransition;
import javafx.animation.Timeline;
import javafx.animation.KeyFrame;
import javafx.animation.KeyValue;
import javafx.geometry.Pos;
import javafx.scene.control.Button;
import javafx.scene.control.Label;
import javafx.scene.effect.DropShadow;
import javafx.scene.image.Image;
import javafx.scene.image.ImageView;
import javafx.scene.layout.*;
import javafx.scene.paint.Color;
import javafx.scene.text.Font;
import javafx.scene.text.Text;
import javafx.util.Duration;

import static com.arkanoid.CONSTANT.WINDOW_HEIGHT;
import static com.arkanoid.CONSTANT.WINDOW_WIDTH;

public class GameOverView extends StackPane {

    private final Button retryButton;
    private final Button menuButton;
    private final VBox gameOverBox;
    private Label scoreLabel;
    private Timeline glowAnimation;

    public GameOverView() {
        setPrefSize(WINDOW_WIDTH, WINDOW_HEIGHT);
        setVisible(false);

        Pane overlay = new Pane();
        overlay.setStyle("-fx-background-color: rgba(0, 0, 0, 0.8);");
        overlay.setPrefSize(WINDOW_WIDTH, WINDOW_HEIGHT);
        getChildren().add(overlay);

        gameOverBox = new VBox(20);
        gameOverBox.setAlignment(Pos.CENTER);
        gameOverBox.setMaxWidth(400);
        gameOverBox.setMaxHeight(450);

        createAnimatedBorder();

        ImageView gameOverTitle = createGameOverTitleImage();
        VBox.setMargin(gameOverTitle, new javafx.geometry.Insets(-20, 0, 10, 0));


        VBox scoreBox = createScoreDisplay();

        retryButton = createButton("BtnRetryNormal", "BtnRetryHover", "RETRY");
        menuButton = createButton("BtnMenuNormal", "BtnMenuHover", "MENU");

        gameOverBox.getChildren().addAll(gameOverTitle, scoreBox, retryButton, menuButton);
        getChildren().add(gameOverBox);
    }

    private void createAnimatedBorder() {

        gameOverBox.setStyle("-fx-background-color: rgba(20, 20, 40, 0.3); " +
                "-fx-border-color: rgba(239, 68, 68, 1); " +
                "-fx-border-width: 3; " +
                "-fx-background-radius: 0; " +
                "-fx-border-radius: 0; " +
                "-fx-padding: 50;");


        DropShadow borderGlow = new DropShadow();
        borderGlow.setColor(Color.rgb(239, 68, 68, 0.8));
        borderGlow.setRadius(20);
        borderGlow.setSpread(0.3);

        // Animation cho viền phát sáng
        glowAnimation = new Timeline(
                new KeyFrame(Duration.ZERO,
                        new KeyValue(borderGlow.radiusProperty(), 15),
                        new KeyValue(borderGlow.spreadProperty(), 0.2)),
                new KeyFrame(Duration.seconds(1.5),
                        new KeyValue(borderGlow.radiusProperty(), 25),
                        new KeyValue(borderGlow.spreadProperty(), 0.4)),
                new KeyFrame(Duration.seconds(3),
                        new KeyValue(borderGlow.radiusProperty(), 15),
                        new KeyValue(borderGlow.spreadProperty(), 0.2))
        );
        glowAnimation.setCycleCount(Timeline.INDEFINITE);

        gameOverBox.setEffect(borderGlow);
    }

    private ImageView createGameOverTitleImage() {
        ImageView gameOverImageView = new ImageView();

        try {
            Image gameOverImage = AssetsManager.getFrames("GameOverTitle")[0];
            gameOverImageView.setImage(gameOverImage);
            gameOverImageView.setFitWidth(300);
            gameOverImageView.setPreserveRatio(true);

            DropShadow textShadow = new DropShadow();
            textShadow.setColor(Color.rgb(239, 68, 68, 0.8));
            textShadow.setRadius(15);
            gameOverImageView.setEffect(textShadow);

        } catch (Exception e) {
            System.out.println("Could not load game over title image, using text fallback");

            Text gameOverText = new Text("GAME OVER");
            gameOverText.setFont(Font.font("Arial", 48));
            gameOverText.setFill(Color.rgb(239, 68, 68));
            gameOverText.setStyle("-fx-font-weight: bold;");

            DropShadow textShadow = new DropShadow();
            textShadow.setColor(Color.rgb(239, 68, 68, 0.8));
            textShadow.setRadius(15);
            gameOverText.setEffect(textShadow);

            ImageView fallbackView = new ImageView();
            fallbackView.setUserData(gameOverText);
        }

        return gameOverImageView;
    }

    private VBox createScoreDisplay() {
        VBox scoreBox = new VBox(8);
        scoreBox.setAlignment(Pos.CENTER);
        scoreBox.setStyle(
                "-fx-background-color: rgba(0, 0, 0, 0.5); " +
                        "-fx-border-color: rgba(239, 68, 68, 0.3); " +
                        "-fx-border-width: 2; " +
                        "-fx-padding: 20; " +
                        "-fx-background-radius: 0; " +
                        "-fx-border-radius: 0;"
        );

        Label titleLabel = new Label("FINAL SCORE");
        titleLabel.setStyle(
                "-fx-font-size: 18px; " +
                        "-fx-font-weight: bold; " +
                        "-fx-text-fill: #ef4444;"
        );

        scoreLabel = new Label("0");
        scoreLabel.setStyle(
                "-fx-font-size: 48px; " +
                        "-fx-font-weight: bold; " +
                        "-fx-text-fill: white; " +
                        "-fx-effect: dropshadow(gaussian, rgba(239, 68, 68, 0.8), 10, 0, 0, 3);"
        );

        scoreBox.getChildren().addAll(titleLabel, scoreLabel);
        return scoreBox;
    }

    public void setGameComplete(boolean isComplete) {
        if (isComplete) {
            // Đổi màu viền sang vàng khi hoàn thành game
            gameOverBox.setStyle("-fx-background-color: rgba(20, 20, 40, 0.3); " +
                    "-fx-border-color: rgba(245, 158, 11, 1); " +
                    "-fx-border-width: 3; " +
                    "-fx-background-radius: 0; " +
                    "-fx-border-radius: 0; " +
                    "-fx-padding: 50;");

            DropShadow goldGlow = new DropShadow();
            goldGlow.setColor(Color.rgb(245, 158, 11, 0.8));
            goldGlow.setRadius(20);
            goldGlow.setSpread(0.3);
            gameOverBox.setEffect(goldGlow);
        } else {
            // Viền đỏ mặc định khi game over
            gameOverBox.setStyle("-fx-background-color: rgba(20, 20, 40, 0.3); " +
                    "-fx-border-color: rgba(239, 68, 68, 1); " +
                    "-fx-border-width: 3; " +
                    "-fx-background-radius: 0; " +
                    "-fx-border-radius: 0; " +
                    "-fx-padding: 50;");

            DropShadow redGlow = new DropShadow();
            redGlow.setColor(Color.rgb(239, 68, 68, 0.8));
            redGlow.setRadius(20);
            redGlow.setSpread(0.3);
            gameOverBox.setEffect(redGlow);
        }
    }

    private Button createButton(String normalKey, String hoverKey, String fallbackText) {
        Button button = new Button();
        button.setBackground(Background.EMPTY);
        button.setBorder(Border.EMPTY);
        button.setStyle("-fx-cursor: hand;");
        button.setMinWidth(250);

        DropShadow shadow = new DropShadow();
        shadow.setColor(Color.rgb(0, 0, 0, 0.5));
        shadow.setRadius(10);
        button.setEffect(shadow);

        try {
            Image normalImg = AssetsManager.getFrames(normalKey)[0];
            Image hoverImg = AssetsManager.getFrames(hoverKey)[0];

            ImageView imageView = new ImageView(normalImg);
            imageView.setFitWidth(250);
            imageView.setFitHeight(50);
            imageView.setPreserveRatio(false);
            button.setGraphic(imageView);

            button.setOnMouseEntered(e -> {
                imageView.setImage(hoverImg);
                ScaleTransition scale = new ScaleTransition(Duration.millis(200), button);
                scale.setToX(1.1);
                scale.setToY(1.1);
                scale.play();
            });

            button.setOnMouseExited(e -> {
                imageView.setImage(normalImg);
                ScaleTransition scale = new ScaleTransition(Duration.millis(200), button);
                scale.setToX(1.0);
                scale.setToY(1.0);
                scale.play();
            });

        } catch (Exception e) {
            System.out.println("Could not load button images for " + normalKey + ", using text button");

            Text text = new Text(fallbackText);
            text.setFont(Font.font("Arial", 24));
            text.setFill(Color.WHITE);
            button.setGraphic(text);
            button.setStyle("-fx-cursor: hand; -fx-background-color: #ef4444; " +
                    "-fx-background-radius: 10; -fx-padding: 15 50;");

            button.setOnMouseEntered(ev -> {
                button.setStyle("-fx-cursor: hand; -fx-background-color: #dc2626; " +
                        "-fx-background-radius: 10; -fx-padding: 15 50;");
                ScaleTransition scale = new ScaleTransition(Duration.millis(200), button);
                scale.setToX(1.1);
                scale.setToY(1.1);
                scale.play();
            });

            button.setOnMouseExited(ev -> {
                button.setStyle("-fx-cursor: hand; -fx-background-color: #ef4444; " +
                        "-fx-background-radius: 10; -fx-padding: 15 50;");
                ScaleTransition scale = new ScaleTransition(Duration.millis(200), button);
                scale.setToX(1.0);
                scale.setToY(1.0);
                scale.play();
            });
        }

        return button;
    }

    public void show() {
        setVisible(true);
        toFront();
        if (glowAnimation != null) {
            glowAnimation.play();
        }
    }

    public void hide() {
        setVisible(false);
        if (glowAnimation != null) {
            glowAnimation.pause();
        }
    }

    public void setScore(int score) {
        scoreLabel.setText(String.valueOf(score));
    }

    public Button getRetryButton() {
        return retryButton;
    }

    public Button getMenuButton() {
        return menuButton;
    }
}